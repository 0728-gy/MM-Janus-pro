...
def generate_single_image(
    ...
    while len(generated_tokens) < image_token_num_per_image:
        ...
        probs_for_e = torch.softmax(final_logits/ temperature, dim=-1)
        ...
        if trigger_backtrack:
            if len(history_hidden_states) <= d:
                ...
                for step in range(d):
                    ...
                    b_probs_for_e = torch.softmax(b_final_logits / temperature, dim=-1)
                    b_log_probs_e = torch.log_softmax(b_final_logits / temperature, dim=-1)
